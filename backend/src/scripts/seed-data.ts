...
async function seed() {
  ...
  try {
    ...
    console.log(`\nTotal: ${savedProducts.length} products`);

    savedProducts.forEach((product) => {
      console.log(`- ${product.name}: ₱${product.price.toFixed(2)} (Stock: ${product.stock})`);
    });
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.error('   - MySQL server is not running, OR');
      console.error('   - MySQL is not accessible on localhost:3306, OR');
      console.error('   - Database credentials are incorrect\n');
      console.error('📝 Please check:');
      console.error('   1. Is MySQL running?');
      console.error('   2. Is the database "mini_ecommerce" created?');
      console.error('   3. Are the credentials correct in this script?');
      console.error('      Current: username=root, password=root, host=localhost:3306\n');
    } else if (error.code === 'ER_BAD_DB_ERROR') {
      console.error('\n🔴 Database Error:');
      console.error('   The database "mini_ecommerce" does not exist.\n');
      console.error('📝 Please create it first:');
      console.error('   CREATE DATABASE mini_ecommerce;\n');
    } else if (error.code === 'ER_ACCESS_DENIED_ERROR') {
      console.error('\n🔴 Authentication Error:');
      console.error('   Wrong username or password.\n');
      console.error('📝 Please update credentials in: backend/src/scripts/seed-data.ts\n');
    } else {
